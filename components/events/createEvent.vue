        ...
        <v-card-title>
          <v-row>
            ...
            <v-col
              ...
              <v-select
                  label="Импорт заявок"
                  :items="importApplicationsItems"
                  v-model="selectedEvent"
                  @update:model-value="importEvent()"
              >
              </v-select>
            </v-col>
          </v-row>
...
const importApplicationsItems = ref()
...
//Статус заявки
const org = ref()
org.value = await $fetch('/api/organizations/get', {
  method: "get"
});
items_organizations.value = org.value.map(item => item.name)
...
application.value = await $fetch('/api/volunteerSupport/request', {
  method: "get"
});
importApplicationsItems.value = application.value.map(item => item.nameEvent)
...
function importEvent() {
  const filtredEvent = ref(application.value.filter(item => item.nameEvent === selectedEvent.value))
  nameEvent.value = filtredEvent.value[0].nameEvent
  organizations.value = filtredEvent.value[0].organizations.map(item => item.name)
  addressEvent.value = filtredEvent.value[0].addressEvent
  descriptionEvent.value = filtredEvent.value[0].descriptionEvent
  date.value = filtredEvent.value[0].date.map(item => new Date(item))
  quantityVolunteer.value = filtredEvent.value[0].quantityVolunteer
  skills.value = filtredEvent.value[0].skills

  clothingVolunteer.value = filtredEvent.value[0].clothingVolunteer
  ageRestrictions.value = filtredEvent.value[0].ageRestrictions
  numberOfFunctional.value = filtredEvent.value[0].fnds.length
  fnds.value = filtredEvent.value[0].fnds.map((item,index)=>{
    return{
      addresses: item.addresses,
      dateFND:item.dateFND.map(date=>new Date(date)),
      descriptionFND:item.descriptionFND,
